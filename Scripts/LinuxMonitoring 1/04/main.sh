...
source config.ini
 ...
 pr1=$column1_background
 pr2=$column1_font_color
 pr3=$column2_background
 pr4=$column2_font_color

if [[ -z $pr1 || -z $pr2 || -z $pr3 || -z $pr4  ]]; then
	pr1=6
	pr2=1
	pr3=6
	pr4=1
elif [[ $pr1 = $pr2 || $pr3 = $pr4 ]]; then
	pr1=6
	pr2=1
	pr3=6
	pr4=1
...
if [[ -n $pr1 && $pr1 =~ ^[0-9]+$ && 
	...
	if [[ $pr1 = $pr2 || $pr3 = $pr4 ]]; then
		echo "Параметр цвета и фона в конф файле введены не верно  они не должны совпадать!"
		echo "Поправь файл config.ini"
    ...
    echo "Вводить параметры не нужно! "
    exit 1
fi
